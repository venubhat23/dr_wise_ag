  ...
  class HealthInsuranceImporter
    ...
    def process_row(row, row_number)
      ...
      if health_insurance.save
        ...
        @errors << "Row #{row_number}: #{health_insurance.errors.full_messages.join(', ')}"
        @skipped_count += 1
      end
    ...
    def normalize_insurance_data(row)
      {
        ...
        insurance_type: row['insurance_type']&.to_s&.strip || 'Individual',
        is_admin_added: true,
    ...
    def duplicate_policy?(insurance_data)
      HealthInsurance.exists?(policy_number: insurance_data[:policy_number])
    end

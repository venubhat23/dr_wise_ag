class Api::V1::PublicController < ActionController::Base
  ...
  def search_affiliates
    begin
      ...
      Rails.logger.info "Public search affiliates called with query: '#{query}', limit: #{limit}"
      ...
      if query.present? && query.strip.length >= 2
        ...
        Rails.logger.info "Search found #{affiliates.count} affiliates matching '#{query}'"
      else
        ...
        affiliates = SubAgent.active
                            ...
                            .map { |agent| { id: agent.id, text: agent.display_name } }
        Rails.logger.info "Returning #{affiliates.count} default affiliates"
      end

      Rails.logger.info "Returning affiliates: #{affiliates}"
      render json: { results: affiliates }
    rescue => e
      Rails.logger.error "Error in public search_affiliates: #{e.message}"

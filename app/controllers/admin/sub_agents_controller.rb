class Admin::SubAgentsController < Admin::ApplicationController
  ...
  # POST /admin/sub_agents/1/record_subscription
  ...
  def record_subscription
    ...
    redirect_back fallback_location: admin_sub_agents_path,
                  notice: "Subscription marked as paid for #{record.display_name.presence || 'Affiliate'} (#{from_date.strftime('%d %b %Y')} - #{to_date.strftime('%d %b %Y')})."
  rescue Date::Error
    redirect_back fallback_location: admin_sub_agents_path, alert: 'Please enter valid From and To dates.'
  rescue ArgumentError, ActiveRecord::RecordInvalid => e
    redirect_back fallback_location: admin_sub_agents_path, alert: "Failed to record subscription: #{e.message}"
  end

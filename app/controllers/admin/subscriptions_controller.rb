...
# expired (locked out of the dashboard / app) or never paid.
class Admin::SubscriptionsController < Admin::ApplicationController
  ...
  # GET /admin/subscriptions?type=ambassadors|affiliates&status=active|expiring_soon|expired|unpaid
  def index
    ...
    order = @tab == 'unpaid' ? { created_at: :desc } : { subscription_expires_at: (@tab == 'expired' ? :desc : :asc) }
